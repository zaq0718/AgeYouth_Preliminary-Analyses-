...
# Recode numeric 0/1/2 (and legacy codes) back into Yes/No/Unable
protect_cols <- c("0_IDF","1_ID","2_Author","3_Year","4_Title")
...
#      str_to_lower(as.character(.)) %in% c("9999", "unable to determine") ~ "Unable to determine",
#      TRUE ~ NA_character_
#    )
#  ))
...
server <- function(input, output, session) {
 ...
 ##################   
    ...
    colnames(summary_tbl) <- recode(colnames(summary_tbl),
                                    "0" = "Yes",
                                    "1" = "No",
                                    "9999" = "Unable to determine",
                                    "NA" = "Not Provided",
                                    .default = colnames(summary_tbl))
